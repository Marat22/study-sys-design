...
Чтобы генералу A понять, что генерал B получил сообщение, генерал B должен отправить ACK(MSG(n)) генералу A. Чтобы генерал B удостоверился в том, что генерал A получил ACK(MSG(n)), генерал A должен тоже отправить ACK(ACK(MSG(n))).
